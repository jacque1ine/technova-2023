...
const home = async(req, res, next) => {
  ...
  const myChallengesCursor = await challengesCol.find({'users.joined': userId});
  const techChallengesCursor = await challengesCol.find({'type': 'tech'});
  const hobbyChallengesCursor = await challengesCol.find({'type': 'hobby'});
